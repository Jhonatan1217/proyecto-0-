...
function grupoInstructorCoor($suf) {
  ...
  $areaId = 'area_' . $suf;
  return '<div class="grupoInstructor hidden space-y-5 pt-4 border-t border-gray-100"><div><label for="modalidad_' . $suf . '" class="label-enterprise">Tipo de instructor</label><div class="relative"><select id="modalidad_' . $suf . '" name="modalidad" required class="select-usuario select-simple input-enterprise pr-10 appearance-none cursor-pointer">' . $selModalidad . '</select>' . $chevron . '</div></div><div><label for="tipo_contrato_' . $suf . '" class="label-enterprise">Tipo de contrato</label><div class="relative"><select id="tipo_contrato_' . $suf . '" name="tipo_contrato" required class="select-usuario select-simple input-enterprise pr-10 appearance-none cursor-pointer">' . $selContrato . '</select>' . $chevron . '</div></div><div class="pt-2"><label class="inline-flex items-center gap-2 cursor-pointer"><input type="checkbox" name="rol_encargado_trimestralizacion" id="' . $cbId . '" value="1" class="rounded border-gray-300 text-[#39A900] focus:ring-[#39A900]"><span class="text-sm text-gray-700">Encargado de trimestralización</span></label></div></div><div class="grupoCoordinador hidden pt-4 border-t border-gray-100"><label for="' . $areaId . '" class="label-enterprise">Área del coordinador</label><input type="text" id="' . $areaId . '" name="area_coordinador" required placeholder="Ingrese el área" class="input-enterprise"></div>';
}
?>
  ...
  <div class="modal-usuario-box bg-white rounded-2xl overflow-hidden">
    ...
    <form id="formUsuario" class="flex flex-col flex-1 min-h-0" novalidate>
      ...
      <div class="modal-usuario-body flex-1"><div class="modal-usuario-form-content space-y-5">
        ...
        <div><label for="tipo_documento_nuevo" class="label-enterprise">Tipo de documento</label><div class="relative"><select id="tipo_documento_nuevo" name="tipo_documento" required class="select-usuario select-simple input-enterprise pr-10 appearance-none cursor-pointer"><?= $selTipoDoc ?></select><?= $chevron ?></div></div>
        ...
        <div><label for="cargo_nuevo" class="label-enterprise">Cargo</label><div class="relative"><select id="cargo_nuevo" name="cargo" class="selectCargoModal select-usuario select-simple input-enterprise pr-10 appearance-none cursor-pointer" required><?= $selCargo ?></select><?= $chevron ?></div></div>
  ...
  <div class="modal-usuario-box bg-white rounded-2xl overflow-hidden">
    ...
    <form id="formEditarUsuario" class="flex flex-col flex-1 min-h-0" novalidate>
      ...
      <div class="modal-usuario-body flex-1"><div class="modal-usuario-form-content space-y-5">
        ...
        <div><label for="tipo_documento_editar" class="label-enterprise">Tipo de documento</label><div class="relative"><select id="tipo_documento_editar" name="tipo_documento" required class="select-usuario select-simple input-enterprise pr-10 appearance-none cursor-pointer"><?= $selTipoDoc ?></select><?= $chevron ?></div></div>
        ...
        <div><label for="cargo_editar" class="label-enterprise">Cargo</label><div class="relative"><select id="cargo_editar" name="cargo" class="selectCargoModal select-usuario select-simple input-enterprise pr-10 appearance-none cursor-pointer" required><?= $selCargo ?></select><?= $chevron ?></div></div>
